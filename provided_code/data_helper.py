...
class UnlabeledDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if self.first_dim == 'sample':
            ...
            image_tensor_i = torch.stack([image[0] for image in images])
            image_tensor_j = torch.stack([image[1] for image in images])
            label_tensor=torch.tensor(labels)
            return (image_tensor_i,image_tensor_j), label_tensor
